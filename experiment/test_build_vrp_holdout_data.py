...
def test_holdout_can_limit_validation_period() -> None:
    events = pd.DataFrame(
        {
            "analysis_eligible": [True, True, True],
            "event_id": ["train", "validation", "future"],
            "origin_site_id": ["A", "A", "A"],
            "destination_site_id": ["B", "B", "B"],
            "service_date": ["2023-06-01", "2023-07-01", "2023-10-01"],
            "departed_at": pd.to_datetime(
                ["2023-06-01 08:00", "2023-07-01 08:00", "2023-10-01 08:00"]
            ),
            "arrived_at": pd.to_datetime(
                ["2023-06-01 09:00", "2023-07-01 09:00", "2023-10-01 09:00"]
            ),
            "distance_km": [10.0, 10.0, 10.0],
            "duration_hours": [1.0, 1.0, 1.0],
            "vehicle_type_name": ["van", "van", "van"],
            "vehicle": ["v1", "v2", "v3"],
            "order_count": [1, 1, 1],
        }
    )
    ...
    layer = build_holdout_layer(
        ...
        min_instance_tasks=1,
        required_instances=1,
        test_end_exclusive=pd.Timestamp("2023-10-01"),
    )

    assert set(layer["test_tasks"]["task_id"]) == {"validation"}
    assert (
        layer["report"]["validation_protocol"]["test_period_end_exclusive"]
        == "2023-10-01"
    )
